...
RSpec.describe DiscourseReactions::ReactionManager do
  ...
  describe ".toggle!" do
    context "when the user has not yet reacted to the post" do
      # TODO (martin) Deny list setting would hook in here for interchange
      context "when the new reaction counts as a PostAction Like via discourse_reactions_reaction_for_like" do
        it "creates a PostAction record" do
      ...
      context "when the new reaction does not count as a PostAction like" do
    ...
    context "when the user already reacted to the Post" do
      context "when the existing reaction was a ReactionUser" do
        ...
        context "when the user has permission to delete the ReactionUser" do
          it "removes the ReactionUser for +1" do
          ...
          it "adds a new ReactionUser record for -1" do
            reaction_manager("-1").toggle!
            expect(
      ...
      context "when the existing reaction counted as a PostAction (Like)" do
        let!(:post_action) do
          Fabricate(
            :post_action,
            post: post,
