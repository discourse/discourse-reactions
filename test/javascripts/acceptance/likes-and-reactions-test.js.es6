...
QUnit.test("Displays correct reactions count", async (assert) => {
  ...
  assert.equal(find('[id="post_1"] .discourse-reactions-counter .reactions-counter').text(), 209);
});

QUnit.test("Reactions list contains reactions sorted by count", async (assert) => {
  await visit("/t/-/topic_with_reactions_and_likes");
  const expectedSequence = "heartangrylaughingopen_mouthcrythumbsdownnose:t2thumbsup"

  const reactions = find('[id="post_1"] .discourse-reactions-counter .discourse-reactions-list .reactions .reaction').text()
